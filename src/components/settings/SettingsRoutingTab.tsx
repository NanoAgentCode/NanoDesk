...
import { MultiSelect, Paper, SimpleGrid, Text, UnstyledButton } from "@mantine/core";
  ...
  return (
    <div className="settings-tab-content model-tab-content">
      <div className="model-header-row">
        <div>
          <h3>智能路由</h3>
          <p className="description description--tight">按路由模式维护独立模型池，再配置模型的任务范围及评分。</p>
        </div>
      ...
      <div className="model-config-grid llm-config-grid">
        <aside className="model-config-list">
          {chatModels.map((item) => (
            ...
            >
              <span className={Object.values(model.routing.assignments).some((ids) => ids.includes(item.id))
