...
import { Fingerprint, Play, RefreshCw, Save, Trash2 } from "lucide-react";
...
export default function SettingsMemoryTab({ workspace, memory, workspaceRef, activeModelId }: SettingsMemoryTabProps) {
  ...
  return (
    <div className="settings-tab-content">
      <h3>用户画像与记忆库</h3>
      <p className="description">用户画像由后台只读生成，可删除不可编辑；普通手工记忆继续支持创建、编辑、启用和删除。</p>
      ...
      <section className="profile-settings-card" aria-labelledby="profile-settings-title">
        <div className="profile-settings-title-row">
          ...
          <label className="profile-toggle"><input type="checkbox" checked={settings.enabled} onChange={(event) => setSettings({ ...settings, enabled: event.currentTarget.checked })} /><span>{settings.enabled ? "启用" : "关闭"}</span></label>
        </div>
        ...
        <div className="profile-settings-actions">{status && status.failed_batches + status.blocked_batches > 0 && <button type="button" onClick={() => void handleRetryFailures()} disabled={!savedEnabled}>重试异常</button>}<button type="button" onClick={() => void handleRunNow()} disabled={!savedEnabled || loading}><Play size={14} />立即检查</button><button type="button" className="primary" onClick={() => void handleSaveSettings()} disabled={saving || (settings.enabled && !settings.model_config_id)}><Save size={14} />{saving ? "保存中" : "保存设置"}</button></div>
      ...
      <section className="user-profile-card" aria-labelledby="user-profile-title">
        <header className="user-profile-header">
          ...
          <div className="profile-header-actions"><button className="user-profile-refresh" type="button" onClick={() => void loadProfile()} disabled={loading} title="刷新"><RefreshCw size={15} className={loading ? "svg-spin" : undefined} /></button><button className="user-profile-refresh danger" type="button" onClick={() => void handleClearProfile()} disabled={!profile?.facts.length} title="清空画像"><Trash2 size={15} /></button></div>
        </header>
