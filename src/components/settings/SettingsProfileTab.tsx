    ...
    <div className="settings-tab-content profile-tab-content">
      ...
      <section className="profile-settings-card" aria-labelledby="profile-settings-title">
        <div className="profile-settings-title-row">
          ...
          <div className="profile-header-actions"><label className="profile-toggle"><input type="checkbox" checked={settings.enabled} onChange={(event) => { setSettings({ ...settings, enabled: event.currentTarget.checked }); setSettingsExpanded(true); }} /><span>{settings.enabled ? "启用" : "关闭"}</span></label><IconTooltipButton className="profile-settings-expand" label={settingsExpanded ? "折叠配置" : "展开配置"} aria-expanded={settingsExpanded} aria-controls="profile-settings-content" onClick={() => setSettingsExpanded((expanded) => !expanded)}><ChevronDown size={17} className={settingsExpanded ? "is-expanded" : undefined} aria-hidden="true" /></IconTooltipButton></div>
        </div>
      ...
      <section className="user-profile-card" aria-labelledby="user-profile-title">
        <header className="user-profile-header">
          ...
          <div className="profile-header-actions"><IconTooltipButton label={generating ? "正在生成用户画像" : generateTip} tooltipOpened={generateTipOpened ? true : undefined} onClick={() => void handleGenerateNow()} disabled={!savedEnabled || loading || generating}>{generating ? <Loader2 size={16} className="svg-spin" /> : <Play size={16} />}</IconTooltipButton><IconTooltipButton label={loading ? "刷新中" : "刷新画像"} onClick={() => void loadProfile()} disabled={loading}><RefreshCw size={16} className={loading ? "svg-spin" : undefined} /></IconTooltipButton><IconTooltipButton label="清空画像" tone="danger" onClick={() => void handleClearProfile()} disabled={!profile?.facts.length}><Trash2 size={16} /></IconTooltipButton><IconTooltipButton className="profile-settings-expand" label={factsExpanded ? "收起条目" : "展开条目"} aria-expanded={factsExpanded} aria-controls="user-profile-facts-content" onClick={() => setFactsExpanded((expanded) => !expanded)} disabled={!profile?.facts.length}><ChevronDown size={17} className={factsExpanded ? "is-expanded" : undefined} aria-hidden="true" /></IconTooltipButton></div>
        </header>
