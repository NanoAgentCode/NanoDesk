...
9. Rust 后端通过 Tauri event `chat-stream` 回传 delta、reasoning_delta 或 error。
10. 前端临时展示流式消息，完成后写入正式 assistant message。
11. 前端解析 assistant 输出中的工具调用，进入审批或结束 run；工具结果续写重新经过同一个上下文准备入口。
