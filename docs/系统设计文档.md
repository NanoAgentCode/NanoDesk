...
| 上下文预算 | `src-tauri/src/context_budget.rs` | 桌面端与 CLI 共用的 Token 估算、模型窗口与输出预留、摘要切点、批次切分和上下文装箱 |
| 上下文准备 | `src/lib/contextPreparation.ts`、`src-tauri/src/context.rs` | 桌面摘要模型调用与持久化编排；两端共用画像、记忆、项目文件和混合检索基础上下文 |
...
6. “用量分析”通过 `get_usage_analysis` 汇总会话库中的会话、消息、模型回答次数和按消息内容估算的 Token 趋势，并结合观测库中的 LLM、MCP、Ops 调用 span 计算平均与 P95 延迟。Token 估算用于本地趋势观察，不作为供应商账单依据。

### 5.11 `nano` 终端客户端

1. `src-tauri/src/bin/nano.rs` 调用共享库中的 `run_cli`，不启动桌面窗口。
2. CLI 默认读取桌面端 `com.nanodesk.desktop` 的 app data；也可用 `--data-dir` 覆盖。
3. 项目模式直接复用领域业务库、会话服务、代码/文档混合索引与召回、基础上下文、上下文预算和 `llm.rs`，会话写入会话库的 `conversations/messages`；临时模式只在进程内保留上下文。
