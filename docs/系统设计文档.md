...
NanoDesk 使用六份活跃 SQLite 数据库；升级用户还会保留一份只读迁移源：

| 数据库 | 职责 | 主要表 |
| --- | --- | --- |
| `nano-agent-config.sqlite3` | 配置数据 | `model_configs`、`mcp_servers`、`ops_servers` |
| `nano-agent-conversations.sqlite3` | 会话数据 | `conversations`、`messages`、`rag_*`、`profile_*`、`user_profile_*` |
| `nano-agent-knowledge.sqlite3` | 知识数据 | `items`、`memories`、`memory_*` |
| `nano-agent-project-index.sqlite3` | 项目索引 | `code_*`、`project_index_*` |
...
| `nano-agent.sqlite3` | 旧库迁移源 | 升级迁移完成后保留但不再写入 |

拆分原因：

- 配置、会话、知识和项目索引可按用途独立备份与排障；旧主库通过可重入事务迁移。
