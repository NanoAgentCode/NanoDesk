...
- AI 对话：多模型配置、服务商模型列表获取、按实际模型标识选择、流式输出、上下文压缩、会话归档。
...
10. 前端解析 assistant 输出中的工具调用，进入审批或结束 run。
...
3. RAG 文件由前端读取文本内容，或调用 `read_absolute_file` 让 Rust 抽取 PDF、docx、pptx、xlsx 等文本。
4. `index_rag_file` 对文本归一化、分块，并调用 embedding API。
5. 主业务库写入 `rag_files`、`rag_chunks`、`rag_embeddings`，并同步维护 `rag_chunks_fts`。
...
4. `save_chat_image_attachment` 在后端校验根目录、扩展名、大小和文件名，把图片保存到 `.nano-agent/uploads/images/`。
...
5. 批准后 `execute_agent_tool_call` 执行文件读写、OCR、命令或 MCP 工具。
6. 执行结果作为用户消息回写对话。
...
2. CLI 默认读取桌面端 `com.nanoagent.desktop` 的 app data；也可用 `--data-dir` 覆盖。
3. 项目模式直接复用主业务库、代码/文档索引和 `llm.rs`，会话写入 `conversations/messages`；临时模式只在进程内保留上下文。
...
1. 持久化会话写入用户消息时，`conversation_store` 在同一主库事务中追加画像观察引用；临时 CLI 会话不收集。
2. 桌面端或 CLI Worker 从主库领取预处理与提取任务，在本地完成候选过滤、去重和批次构建。
...
6. 设置中的“用户画像”页负责开关、模型与预算、处理状态、重试、立即生成、单条删除和清空；“立即生成”会将当前有效候选强制组批并等待提取完成，但不绕过前台启动门和调用预算。“记忆库”页只管理普通手工记忆。
...
| 数据库 | 职责 | 主要表 |
| --- | --- | --- |
| `nano-agent.sqlite3` | 主业务数据 | `items`、`model_configs`、`mcp_servers`、`conversations`、`messages`、`rag_*`、`memories`、`profile_*`、`user_profile_*`、`ops_servers`、`code_*`、`project_index_*` |
...
Windows 默认 app data 位于 `%APPDATA%\com.nanoagent.desktop`。当前模型/embedding API key、MCP env/headers、Ops 密码和 Tavily API key 没有应用层静态加密，备份或复制 app data 时必须按敏感凭据处理。
...
- 图片附件保存到根目录内 `.nano-agent/uploads/images/`，读取预览和 OCR 都只允许访问受控相对路径。
...
- 新业务对象：优先放入主业务库；如果是运行过程或高频日志，考虑独立库。
