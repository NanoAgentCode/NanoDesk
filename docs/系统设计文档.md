...
| 层级 | 关键文件 | 职责 |
| --- | --- | --- |
| UI | `src/App.tsx`、`src/components/*` | 页面布局、侧栏、聊天区、设置弹窗、观测面板、Ops 工作台 |
...
| 主业务存储 | `src-tauri/src/db.rs` | items、model_configs、mcp_servers、conversations、messages、RAG、memories、memory vectors/graph、ops_servers |
...
5. 主业务库写入 `rag_files`、`rag_chunks`、`rag_embeddings` 和 `rag_chunks_fts`。
6. 对话发送前调用 `search_rag_context`，结合 FTS 和向量匹配召回片段。
