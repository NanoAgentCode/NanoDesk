...
复杂任务还可输出结构化 `<task_plan>` JSON。计划包含一个可验证目标和 2 到 12 个稳定 ID 的步骤，步骤状态限定为 `pending`、`in_progress`、`completed`、`blocked`、`skipped`，并且同时最多只有一个步骤处于执行中。计划可与单个工具调用一起输出，并在每次工具结果返回后更新；简单问答和单步操作不生成计划，避免增加无效流程。
...
最新任务计划以 JSON 保存在所属 run 中，每次更新同时生成 `planning` step，记录目标、完成数和完整计划。聊天消息展示当次计划，Agent Runtime 展示持久化后的最新版本；应用重启后无需打开同话即可恢复计划视图。计划只描述执行状态，不替代工具审批、结果验证或失败恢复状态机。
...
上下文预算的权威算法位于 Rust `context_budget.rs`，桌面端通过 `contextPreparation.ts` 调用它并负责摘要模型调用与持久化，CLI 则直接调用同一核心。初次回答、工具结果续写和 CLI 长对话因此采用相同的窗口预留、裁剪、摘要切点与装箱规则。
